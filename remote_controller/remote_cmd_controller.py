...
def parse_args():
    ...
    logging.debug(
        'parse_args: url({}), userId({}), guid({}), type({}), proxy({}), verbose({}), eof-wait({}), timings({})'.format(
            args.url, args.userId, args.guid, args.type, args.proxy, args.verbose, args.eof_wait, args.timings))
    return args
...
def main():
    ...
    while True:
        ...
        if console.connect:
            ...
            break
        ws.send(connect_msg)
        logging.debug('{}: re_connect -- send: {}.'.format(
            tag, connect_msg))
