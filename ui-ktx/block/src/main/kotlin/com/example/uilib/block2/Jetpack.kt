...
open class StrongLiveData<T> : MutableLiveData<T> {
    ...
    protected open val myStateListener: StateListener? = null
    ...
    private fun judgeEqual(t: T?): Boolean {
        if (useEquals) {
            val now = value
            if (t == now) {
                return true
            }
            if (t is Comparable<*> && now != null) {
                t as Comparable<T>
    ...
    override fun onActive() = myStateListener?.onActive() ?: Unit
    override fun onInactive() = myStateListener?.onInactive() ?: Unit
