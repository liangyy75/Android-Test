...
DEFAULT_SERVER_URL = 'ws://172.18.33.203:9001'
...
def _define_widget(target: Union[QtWidgets.QWidget, QtWidgets.QLineEdit, QtWidgets.QLabel],
                   geometry: List[
                       int], font: QtGui.QFont, object_name: str) -> Union[
    QtWidgets.QWidget, QtWidgets.QLineEdit, QtWidgets.QLabel]:
    """geometry -- [left: int, top: int, width: int, height: int]: list[int]"""
...
class FocusLabel(QtWidgets.QLabel):
    ...
    def set_target(self, target: QtWidgets.QWidget) -> None:
        ...
        logging.debug('{}: {} -- set target -- {}'.format(self.tag, self.name1, self.name2))

    def mousePressEvent(self, ev: QtGui.QMouseEvent) -> None:
        if self.target is not None:
            self.target.setFocus()
            logging.debug('{}: {} -- set focus -- {}'.format(self.tag, self.name1, self.name2))
        else:
            logging.debug('{}: cannot set focus, target is None'.format(self.tag))
...
class AbsRemoteController(metaclass=ABCMeta):
...
class TestRemoteController(AbsRemoteController):

    def __init__(self):
        super(TestRemoteController, self).__init__("testReq", "testRes")
    ...
    def transform(self, str_data: str, data_type: str = 'json') -> str:
        ...
        if data_type == 'json':
            try:
                ...
                msg = str(ex) + '\n'
                logging.error(
                    '{}: write\'s json.decoder.JSONDecodeError: {}'.format(self.tag, str(ex)))
        elif data_type == 'str':
...
class WebSocketClient(QtCore.QThread):
        ...
        if not self.has_finished:
            ...
            if self.ws is not None:
                try:
                    ...
                    logging.error('{}: _finish\' exception: {}'.format(self.tag, ex))
            self.ws = None
            ...
            logging.debug('{}: __destroy__ with url({}), uid({}), guid({}), msg_type({})'.format(
                self.tag, self.url, self.uid, self.guid, self.msg_type))
            self.sinOut.emit(
                self.handler.transform('connection has been closed: {}.'.format(reason), 'str'))
        else:
    ...
    def recv(self) -> Union[Tuple[int, bytes], Tuple[int, None]]:
        try:
            ...
            logging.error('{}: recv\'s websocket.WebSocketException: {}'.format(self.tag, str(ex)))
        ...
        while self.working:
            try:
                ...
                if op_code in OP_CODE_DATA:
                    msg = data
                if msg is not None:
                    logging.debug('{}: recv result -- write: {}.'.format(self.tag, msg))
                    self.sinOut.emit(self.handler.transform(msg))
                if op_code == websocket.ABNF.OPCODE_CLOSE:
                    self.working = False
            except Exception as ex:
                str_ex = str(ex)
                logging.error('{}: recv_ws\' exception: {}'.format(self.tag, str_ex))
                # if str_ex == 'socket is already closed.' or str_ex == '[WinError 10054] 远程 主机强迫关闭了一个现有的连接。':
                self.working = False
    ...
    def connect(self) -> Tuple[str, bool]:
        ...
        while True:
            ...
            if time.time() - begin_connect > 10:
                ...
                break
            self.ws.send(connect_msg)
            logging.debug('{}: re_connect -- send: {}.'.format(self.tag, connect_msg))
        self.has_connected = self.handler.connect
        logging.debug('{}: finish connection, result: {}.'.format(self.tag, self.has_connected))
    ...
    def _check_params(self, flag: bool) -> Tuple[str, bool]:
        ...
        if not flag:
            ...
            logging.debug('{}: check_params -- wrong msg: {}'.format(self.tag, wrong_msg))
    ...
    def run(self):
        ...
        if flag:
            try:
                logging.debug('{}: begin dealing with commands'.format(self.tag))
                while self.working:
                    ...
                    self.sinOut.emit(self.handler.transform(
                        '> {}'.format(command), 'str'))
                    logging.debug('{}: send command: {}'.format(self.tag, command))
                    self.ws.send(parse_command(self.msg_type, command))
                msg = 'after running'
            except Exception as ex:
                msg = str(ex)
                self.sinOut.emit(self.handler.transform(msg, 'str'))
                logging.error('{}: send command\'s Exception: {}'.format(self.tag, msg))
        else:
...
class UiForm(object):
    ...
    def setup_ui(self, form: QtWidgets.QWidget):
        ...
        self.form.resize(1080, 630)
        self.form.setFont(font)

        logging.debug('{}: init {}\'s widgets'.format(self.tag, 'url'))
        self.url_label = _define_label(FocusLabel(form), [30, 20, 70, 20], font, 9, 'url_label')
        self.url_edit = _define_edit(QtWidgets.QLineEdit(form), [110, 20, 200, 20], font, '',
                                     'url_edit')
        logging.debug('{}: init {}\'s widgets'.format(self.tag, 'uid'))
        self.uid_label = _define_label(FocusLabel(form), [330, 20, 70, 20], font, 9, 'uid_label')
        self.uid_edit = _define_edit(QtWidgets.QLineEdit(form), [410, 20, 200, 20], font, '',
                                     'uid_edit')
        logging.debug('{}: init {}\'s widgets'.format(self.tag, 'type'))
        self.type_label = _define_label(FocusLabel(form), [630, 20, 80, 20], font, 9, 'type_label')
        self.type_combo_box = _define_widget(QtWidgets.QComboBox(form), [720, 20, 80, 22], font,
                                             'type_combo_box')
        logging.debug('{}: init {}\'s widgets'.format(self.tag, 'guid'))
        self.guid_label = _define_label(FocusLabel(form), [30, 60, 70, 20], font, 9, 'guid_label')
        self.guid_edit = _define_edit(QtWidgets.QLineEdit(form), [110, 60, 501, 20], font, '',
                                      'guid_edit')
        self.connect_btn = _define_widget(QtWidgets.QPushButton(form), [960, 20, 91, 23], font,
                                          'connect_btn')
        self.stop_btn = _define_widget(QtWidgets.QPushButton(form), [860, 20, 91, 23], font,
                                       'stop_btn')

        logging.debug('{}: init {}\'s widgets'.format(self.tag, 'output_text'))
        self.output_text_browser = _define_widget(
            QtWidgets.QTextBrowser(form), [30, 140, 1020, 475], font, 'output_text_browser')
        self.output_text_browser.document().setMaximumBlockCount(3000)
        self.clear_btn = _define_widget(QtWidgets.QPushButton(form), [860, 60, 91, 23], font,
                                        'clear')
        self.save_btn = _define_widget(QtWidgets.QPushButton(form), [960, 60, 91, 23], font, 'save')

        logging.debug('{}: init {}\'s widgets'.format(self.tag, 'command'))
        self.command_edit = _define_edit(HistoryLineEdit(form), [110, 100, 691, 20], font, '',
                                         'command_edit')
        self.command_label = _define_label(FocusLabel(form), [30, 100, 70, 20], font, 9,
                                           'command_label')
        self.send_btn = _define_widget(QtWidgets.QPushButton(form), [860, 100, 91, 23], font,
                                       'send')
    ...
    def re_translate_ui(self, form):
        ...
        self.url_edit.setText(_translate('Form', DEFAULT_SERVER_URL))
    ...
    def add_type(self, msg_type: str):
        ...
        logging.debug('{}: call add_type(msg_type: {})'.format(self.tag, msg_type))
    ...
    def init_events(self):
        ...
        self.clear_btn.clicked.connect(self.text_browser_clear)
        self.save_btn.clicked.connect(self.text_browser_save)
    ...
    def text_browser_save(self):
        ...
        file_name, _ = QtWidgets.QFileDialog.getSaveFileName(
            self.form, "QFileDialog.getSaveFileName()", "", "All Files (*);;Text Files (*.txt)",
            options=options)
        with open(file_name, 'w+', encoding='utf-8') as file:
            file.write(self.output_text_browser.toPlainText())
        logging.debug("{}：call text_browser_save".format(self.tag))

    def text_browser_clear(self):
        self.output_text_browser.clear()
        logging.debug("{}：call text_browser_clear".format(self.tag))
    ...
    def text_browser_write(self, msg: str):
        ...
        if len(msg) > 0:
            ...
            logging.debug('{}：call text_browser_write(str: {})'.format(self.tag, msg))
    ...
    def connect(self):
        ...
        if self.websocket_client is not None and self.websocket_client.working \
            ...
            logging.debug('{}: {}, therefore terminate connection.'.format(self.tag, msg))
            return
        if self.websocket_client is not None:
            self._finish_websocket_client(
                '{}: {}',
                'new connection is required, but last connection is still alive, now close it.')
        self.text_browser_write(
            'starting connect {} with userId: {}, guid: {} and msgType: {}\n'.format(
                url, uid, guid, msg_type))
    ...
    def send_command(self):
        ...
        if self.websocket_client is None or not self.websocket_client.has_connected \
            ...
            logging.warning('{}: sending command before connection is not allowed'.format(self.tag))
            return
...
# TODO: [Qt Designer 设计PyQt5界面时自适应设置](https://blog.csdn.net/loovelj/article/details/79529625)
...
# uid: 50042533; guid: 0e74af97aa48135d0c5528db29dbb6fe; url: ws://172.18.33.203:9001
