...
namespace json11 {
    ...
    static void dump(double value, string &out) {
        if (std::isfinite(value)) {
            char buf[32]; // NOLINT(cppcoreguidelines-avoid-magic-numbers)
    ...
    Json::Json() noexcept                  : m_ptr(statics().null) {}  // null
    Json::Json(std::nullptr_t) noexcept    : m_ptr(statics().null) {}  // null
    Json::Json(double value) : m_ptr(make_shared<JsonDouble>(value)) {}  // double
    Json::Json(int value) : m_ptr(make_shared<JsonInt>(value)) {}  // int
    Json::Json(bool value) : m_ptr(value ? statics().t : statics().f) {}  // bool
    Json::Json(const string &value) : m_ptr(make_shared<JsonString>(value)) {}  // string
    Json::Json(string &&value) : m_ptr(make_shared<JsonString>(move(value))) {}  // string
    Json::Json(const char *value) : m_ptr(make_shared<JsonString>(value)) {}  // char*
    Json::Json(const Json::array &values) : m_ptr(make_shared<JsonArray>(values)) {}  // array
    Json::Json(Json::array &&values) : m_ptr(make_shared<JsonArray>(move(values))) {}  // array
    Json::Json(const Json::object &values) : m_ptr(make_shared<JsonObject>(values)) {}  // object
    Json::Json(Json::object &&values) : m_ptr(make_shared<JsonObject>(move(values))) {}  // object

    /* Accessors */

    Json::Type Json::type() const { return m_ptr->type(); }  // type
    double Json::number_value() const { return m_ptr->number_value(); }  // number
    int Json::int_value() const { return m_ptr->int_value(); }  // int
    bool Json::bool_value() const { return m_ptr->bool_value(); }  // bool
    const string &Json::string_value() const { return m_ptr->string_value(); }  // string
    const vector<Json> &Json::array_items() const { return m_ptr->array_items(); }  // array
    const map<string, Json> &Json::object_items() const { return m_ptr->object_items(); }  // object
    ...
    const Json &JsonObject::operator[](const string &key) const {
        auto iterator = m_value.find(key);
        return (iterator == m_value.end()) ? static_null() : iterator->second;
    }

    const Json &JsonArray::operator[](size_t i) const {
        return i >= m_value.size() ? static_null() : m_value[i];
    }
    ...
    namespace {
        ...
        struct JsonParser final {
            ...
            /* Expect that 'str' starts at the character that was just read. If it does, advance the input and return res. If not, flag an error. */
